...
const simpleRulesetDeclarations: Array<keyof typeof rulesetDirectives> = ['extends', 'states', 'mixin'];
            ...
            importDeclarations.forEach(name => {
                if (
                    parentSelector!.nodes!.every(
                        n => (isDeclaration(n) && importDirectives[name] !== n.prop) || isComment(n)
                    ) &&
                    importDirectives[name].indexOf(fullLineText.trim()) === 0
                ) {
                    res.push(
                        importInternalDirective(name, createDirectiveRange(position, fullLineText, lineChunkAtCursor))
                    );
                }
            });
            return res;
...
export const RulesetInternalDirectivesProvider: CompletionProvider & { isSimpleSelector: (sel: string) => boolean } = {
    provide({
        ...
        if (parentSelector && !(parentSelector.selector === ':import' || parentSelector.selector === ':vars')) {
            if (
                parentSelector.nodes!.every(
                    n => (isDeclaration(n) && rulesetDirectives.mixin !== n.prop) || isComment(n)
                ) &&
                rulesetDirectives.mixin.indexOf(fullLineText.trim()) === 0
            ) {
                res.push(
                    rulesetInternalDirective('mixin', createDirectiveRange(position, fullLineText, lineChunkAtCursor))
                );
            }
            if (this.isSimpleSelector(parentSelector.selector) && !isMediaQuery) {
                simpleRulesetDeclarations
                    ...
                    .forEach(name => {
                        if (
                            parentSelector!.nodes!.every(
                                n => (isDeclaration(n) && rulesetDirectives[name] !== n.prop) || isComment(n)
                            ) &&
...
export const GlobalCompletionProvider: CompletionProvider = {
    provide({ parentSelector, fullLineText, position, lineChunkAtCursor }: ProviderOptions): Completion[] {
        if (!parentSelector && !lineChunkAtCursor.endsWith('::') && !isBetweenChars(fullLineText, position, '(', ')')) {
            ...
            return [
                globalCompletion(
                    new ProviderRange(new ProviderPosition(position.line, position.character - offset), position)
                )
            ];
        } else {
            return [];
...
export const SelectorCompletionProvider: CompletionProvider = {
    provide({
        ...
        if (!parentSelector && (lineChunkAtCursor === ':' || !lineChunkAtCursor.endsWith(':'))) {
            ...
            comps.push(
                ...keys(meta.customSelectors).map(c =>
                    classCompletion(c, createDirectiveRange(position, fullLineText, lineChunkAtCursor), true)
                )
            );
                ...
                .filter(
                    ms =>
                        (meta.mappedSymbols[ms]._kind === 'import' &&
                            (meta.mappedSymbols[ms] as ImportSymbol).import.fromRelative.endsWith('st.css')) ||
...
export const PseudoElementCompletionProvider: CompletionProvider = {
    provide({
        ...
        if (!parentSelector && resolved.length > 0 && !isBetweenChars(fullLineText, position, '(', ')')) {
            ...
            if (
                !filter &&
                lineChunkAtCursor.split('::').length > 1 &&
                last(lineChunkAtCursor.split('::')) === scope.name
            ) {
...
export const StateSelectorCompletionProvider: CompletionProvider = {
    provide({
        ...
        if (!parentSelector && !lineChunkAtCursor.endsWith('::') && !isBetweenChars(fullLineText, position, '(', ')')) {
                        ...
                        keys(symbol[valueMapping.states]).forEach(k => {
                            if (
                                ...
                                (k.slice(0, -1).startsWith(lastSelectoid.replace(':', '')) ||
                                    ...
                                    nativePseudoClasses.indexOf(lastSelectoid.replace(':', '')) !== -1 ||
            ...
            const states = keys(newStates).map(k => {
            ...
            if (lastSelectoid.startsWith(':')) {
                ...
                if (Object.keys(resolvedStates).length) {
                    ...
                    if (resolvedStateNode) {
                        const resolvedState: StateParsedValue = (resolvedStateNode as any).symbol[valueMapping.states][
                            stateName
                        ];
    ...
    provide({ fullLineText, position, meta, styl }: ProviderOptions): Completion[] {
        if (isInValue(fullLineText, position)) {
            const inner = fullLineText
                ...
                .slice(fullLineText.slice(0, fullLineText.indexOf(')', position.character) + 1).lastIndexOf('('))
                .replace('(', '')
                .replace(')', '')
                .trim();
...
function createCodeMixinCompletion(name: string, lastName: string, position: ProviderPosition, meta: StylableMeta) {
    return codeMixinCompletion(
        name,
        new ProviderRange(new ProviderPosition(position.line, position.character - lastName.length), position),
