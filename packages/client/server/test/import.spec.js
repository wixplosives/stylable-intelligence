...
describe('Imports', function () {
    it('should complete :import at top level after ""', function () {
        debugger;
        return asserters.getCompletions('imports/top-level-no-chars.css', true).then(function (asserter) {
            asserter.suggested([
    ...
    it('should complete :import at top level after ":"', function () {
        return asserters.getCompletions('imports/top-level-colon.css', true).then(function (asserter) {
            asserter.suggested([
                asserters.importCompletion
            ]);
            asserter.notSuggested([
    ...
    it('should complete :import at top level even if exists', function () {
        return asserters.getCompletions('imports/top-level-import-exists.css', true).then(function (asserter) {
            asserter.suggested([
                asserters.importCompletion,
            ]);
            asserter.notSuggested([
    ...
    it('should not complete :import after ::', function () {
        return asserters.getCompletions('imports/top-level-colon-colon.css', true).then(function (asserter) {
            asserter.suggested([]);
            asserter.notSuggested([
                asserters.importCompletion,
                asserters.rootCompletion,
    ...
    it('should not complete :import inside selectors', function () {
        return asserters.getCompletions('imports/inside-simple-selector.css', true).then(function (asserter) {
            asserter.suggested([]);
            asserter.notSuggested([
    ...
    it('should complete -st-from, -st-default, -st-named inside import statements', function () {
        return asserters.getCompletions('imports/inside-import-selector.css', true).then(function (asserter) {
            asserter.suggested([
                asserters.importFromDirectiveCompletion,
                asserters.importDefaultDirectiveCompletion,
                asserters.importNamedDirectiveCompletion
    ...
    it('should not complete -st-from, -st-default, -st-named inside import statements when exists', function () {
        return asserters.getCompletions('imports/inside-import-selector-with-fields.css', true).then(function (asserter) {
            asserter.notSuggested([
                asserters.importFromDirectiveCompletion,
                asserters.importDefaultDirectiveCompletion,
                asserters.importNamedDirectiveCompletion,
    ...
    it('completes name imported as default when a following ; exists', function () {
        return asserters.getCompletions('imports/st-extends-with-semicolon.css', true).then(function (asserter) {
            asserter.suggested([
                asserters.extendsCompletion('Comp')
            ]);
            asserter.notSuggested([
