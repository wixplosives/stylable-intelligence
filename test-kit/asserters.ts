...
export function getDefinition(fileName: string): Thenable<ProviderLocation[]> {
    ...
    let pos = getCaretPosition(src);
    src = src.replace('|', "");
    return provider.getDefinitionLocation(src, pos, fullPath).then((res: ProviderLocation[]) => {
        return res;
    })
}
...
export function getReferences(fileName: string, pos: ProviderPosition): Location[] {
    ...
    return provider.getRefs({context: {includeDeclaration: true}, position: pos, textDocument: doc})
}
...
export function getSignatureHelp(fileName: string, prefix: string): SignatureHelp | null {
    ...
    src = src.replace('|', prefix);
    pos.character += prefix.length;
    return provider.getSignatureHelp(src, pos, fullPath, ParameterInformation);
}
...
const provider = new Provider(new Stylable('/', createFs(docsFs, true), () => ({default: {}})), wrappedTs, docsFs);
