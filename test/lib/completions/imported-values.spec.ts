    ...
    describe('from path', () => {
        ...
        [' ' + str1, ' ' + str2].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it('completes default and named imports in -st-extends, with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters.getCompletions('imports/st-extends.st.css', prefix);
        ...
        [str1, str3].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it(
                    'completes named and default imports as initial selectors, with prefix ' + prefix + ' ',
                    async () => {
                        const rng = createRange(6, 0, 6, i);
                        const asserter = await asserters.getCompletions('imports/st-extends-selectors.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
    ...
    describe('from node_modules package', () => {
        ...
        [defaultComp, namedComp].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it('completes default and named imports in -st-extends, with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters.getCompletions('imports/from-package/st-extends.st.css', prefix);
                    const exp: Array<Partial<Completion>> = [];
                    const notExp: Array<Partial<Completion>> = [];
                    exp.push(createComp(a[j], rng, realPath));
                    if (prefix.length <= 1) {
