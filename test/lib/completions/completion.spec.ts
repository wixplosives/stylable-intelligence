...
describe('Completions', () => {
    describe('Stylesheet Top Level', () => {
        it('should complete ONLY import and vars directive, root and existing classes at top level', async () => {
            const asserter = await asserters.getCompletions('general/top-level-existing-classes.st.css');
            asserter.suggested([
        ...
        it('should not complete broken classes at top level', async () => {
            const asserter = await asserters.getCompletions('general/top-level-existing-classes-broken.st.css');
            asserter.suggested([
                asserters.importDirectiveCompletion(createRange(3, 0, 3, 0)),
                asserters.customSelectorDirectiveCompletion(createRange(3, 0, 3, 0)),
                asserters.stScopeDirectiveCompletion(createRange(3, 0, 3, 0)),
        ...
        it('should complete named imports used locally only once', async () => {
            const asserter = await asserters.getCompletions('general/top-level-import-and-local.st.css');
            asserter.suggested([
    ...
    describe('Multiple Files', () => {
        it('complete states for localy imported component', async () => {
            const asserter = await asserters.getCompletions('states/locally-imported-component.st.css');
            asserter.suggested([
        ...
        it('complete states for localy imported component (including local states)', async () => {
            const asserter = await asserters.getCompletions('states/locally-imported-component-with-states.st.css');
            asserter.suggested([
        ...
        it('complete states for localy imported component ( recursive )', async () => {
            const asserter = await asserters.getCompletions('states/locally-imported-component-recursive.st.css');
            asserter.suggested([
