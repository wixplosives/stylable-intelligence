        ...
        describe('value()', () => {
            'value('.split('').map((c, i) => {
                ...
                it('should be completed inside rule value, with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters.getCompletions('variables/local-vars.st.css', prefix);
                    asserter.suggested([asserters.valueDirective(createRange(6, 10, 6, 11 + i))]);
                });

                it(
                    'should be completed inside rule value when other values exist, with prefix ' + prefix + ' ',
                    async () => {
                        const asserter = await asserters
                            .getCompletions('variables/local-vars-several-values.st.css', prefix);
                        asserter.suggested([asserters.valueDirective(createRange(6, 25, 6, 26 + i))]);
                    }
                );

                it(
                    'should be completed inside rule value inside a complex selector, with prefix ' + prefix + ' ',
                    async () => {
                        const asserter = await asserters.getCompletions('variables/complex-selector.st.css', prefix);
                        asserter.suggested([asserters.valueDirective(createRange(15, 10, 15, 11 + i))]);
                    }
                );
            });
            ...
            it('should not be completed inside other value()', async () => {
                const asserter = await asserters.getCompletions('variables/inside-value-local-vars.st.css');
        ...
        describe('Inside value()', () => {
            ...
            str1.split('').forEach((c, i) => {
                ...
                it('Local variables should be completed, with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters
                        .getCompletions('variables/inside-value-local-vars.st.css', prefix);
                    asserter.suggested([
                ...
                it('Imported variables should be completed, with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters
                        .getCompletions('variables/inside-value-imported-vars.st.css', prefix);
                    asserter.suggested([
                ...
                it('Variable being defined should not be completed, with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters
                        .getCompletions('variables/inside-value-defined-var.st.css', prefix);
                    asserter.suggested([
        ...
        describe('value()', () => {
            ...
            importedValue.split('').forEach((c, i) => {
                ...
                it('completes named variable imports in a declaration , with prefix ' + prefix + ' ', async () => {
                    const asserter = await asserters.getCompletions('imports/from-package/value.st.css', prefix);
                    asserter.suggested([
                        asserters.valueCompletion(
                            importedValue,
