...
describe('Named Values', () => {
    ...
    [str1, str2, str3, str4].forEach((str, _j, a) => {
        str.split('').forEach((_c, i) => {
            ...
            it(
                'completes classes and vars from imported file after -st-named, with prefix ' + prefix + ' ',
                async () => {
                    const rng = createRange(2, 15, 2, 15 + i);
                    const asserter = await asserters.getCompletions('named/st-named.st.css', prefix);
            ...
            it('completes names after single value, with prefix ' + prefix + ' ', async () => {
                ...
                const asserter = await asserters.getCompletions('named/st-named-single-value.st.css', prefix);
                const exp: Array<Partial<Completion>> = [];
                const notExp: Array<Partial<Completion>> = [];
                if (prefix.length === 0) {
                    a.forEach(c => {
            ...
            it('completes names after multiple values, with prefix ' + prefix + ' ', async () => {
                ...
                const asserter = await asserters.getCompletions('named/st-named-multi-values.st.css', prefix);
                const exp: Array<Partial<Completion>> = [];
                const notExp: Array<Partial<Completion>> = [];
                if (prefix.length === 0) {
                    a.forEach(c => {
            ...
            it('completes names on second line, with prefix ' + prefix + ' ', async () => {
                ...
                const asserter = await asserters.getCompletions('named/st-named-multi-line.st.css', prefix);
                const exp: Array<Partial<Completion>> = [];
                const notExp: Array<Partial<Completion>> = [];
                if (prefix.length === 0) {
                    a.forEach(c => {
    ...
    [str5, str6].forEach(str => {
        str.split('').forEach((_c, i) => {
            ...
            it('Completes names of functions from JS imports, with prefix ' + prefix + ' ', async () => {
                const asserter = await asserters.getCompletions('named/st-named-mixin.st.css', prefix);
                const exp: Array<Partial<Completion>> = [];
                const notExp: Array<Partial<Completion>> = [];
                if (prefix.length <= 1) {
