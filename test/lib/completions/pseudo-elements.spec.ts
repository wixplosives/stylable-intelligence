...
describe('Pseudo-elements', () => {
    describe('Deafult import used as tag', () => {
        ...
        [str1, str2].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it(
                    'should complete pseudo-element ' + a[j] + ' after CSS state with prefix: ' + prefix + ' ',
                    async () => {
                        const rng = createRange(9, 10, 9, 10 + i);
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/default-import-as-tag-css-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
                ...
                it(
                    'should complete pseudo-element ' + a[j] + ' after imported state with prefix: ' + prefix + ' ',
                    async () => {
                        const rng = createRange(9, 10, 9, 10 + i);
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/default-import-as-tag-imported-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
    ...
    describe('Deafult import extended by class', () => {
        ...
        [str1, str2].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it(
                    'should complete pseudo-element ' + a[j] + ' after local state with prefix: ' + prefix + ' ',
                    async () => {
                        const rng = createRange(11, 10, 11, 10 + i);
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/default-import-extended-local-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
                ...
                it(
                    ...
                    async () => {
                        ...
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/default-import-extended-css-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
                ...
                it(
                    ...
                    async () => {
                        const rng = createRange(10, 12, 10, 12 + i);
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/default-import-extended-imported-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
    ...
    describe('Named import extended by class', () => {
        ...
        [str1, str2].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it('should complete pseudo-element ' + a[j] + ' after class with prefix: ' + prefix + ' ', async () => {
                    ...
                    const asserter = await asserters
                        .getCompletions('pseudo-elements/named-import-extended.st.css', prefix);
                    const exp: Array<Partial<Completion>> = [];
                    const notExp: Array<Partial<Completion>> = [];
                    exp.push(createCompletion(a[j], rng));
                    if (prefix.length <= 2) {
                ...
                it(
                    ...
                    async () => {
                        ...
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/named-import-extended-local-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
                ...
                it(
                    ...
                    async () => {
                        ...
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/named-import-extended-css-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
        ...
        it('should not complete root pseudo-elements on class extending named import', async () => {
            const asserter = await asserters.getCompletions('pseudo-elements/named-import-extended.st.css');
    ...
    describe('Recursive imports', () => {
        ...
        [str1, str2].forEach((str, j, a) => {
            str.split('').forEach((_c, i) => {
                ...
                it(
                    'should complete pseudo-element ' + str + ' after pseudo-element with prefix: ' + prefix + ' ',
                    async () => {
                ...
                it(
                    'should complete pseudo-element ' + str + ' after CSS state with prefix: ' + prefix + ' ',
                    async () => {
                        const rng = createRange(10, 16, 10, 16 + i);
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/recursive-import-3-css-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
                ...
                it(
                    ...
                    async () => {
                        ...
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/recursive-import-3-imported-state.st.css', prefix);
                        const exp: Array<Partial<Completion>> = [];
                        const notExp: Array<Partial<Completion>> = [];
                        exp.push(createCompletion(a[j], rng));
                        if (prefix.length <= 2) {
                ...
                it(
                    ...
                    async () => {
                        ...
                        const asserter = await asserters
                            .getCompletions('pseudo-elements/recursive-import-3-pseudo-element-exists.st.css', prefix);
                        const notExp: Array<Partial<Completion>> = [];
                        notExp.push(createCompletion(a[0], rng));
                        notExp.push(createCompletion(a[1], rng));
                        asserter.notSuggested(notExp);
    ...
    describe('Deep Recursive imports', () => {
        ...
        str.split('').forEach((_c, i) => {
            ...
            it('should complete pseudo-element ' + str + ' in deep chain with prefix: ' + prefix + ' ', async () => {
                const asserter = await asserters
                    .getCompletions('pseudo-elements/recursive-import-3-deep.st.css', prefix);
                const exp: Array<Partial<Completion>> = [];
                const notExp: Array<Partial<Completion>> = [];
        ...
        str1.split('').forEach((_c, i) => {
            ...
            it('should complete state ' + str1 + ' in deep chain with prefix: ' + prefix + ' ', async () => {
                const asserter = await asserters
                    .getCompletions('pseudo-elements/recursive-import-3-deep.st.css', prefix);
                const exp: Array<Partial<Completion>> = [];
                const notExp: Array<Partial<Completion>> = [];
        ...
        str.split('').forEach((_c, i) => {
            ...
            it(
                ...
                async () => {
                    ...
                    const asserter = await asserters
                        .getCompletions('pseudo-elements/default-import-with-native-element.st.css', prefix);
                    const exp: Array<Partial<Completion>> = [];
                    exp.push(createCompletion(str, rng));
                    asserter.suggested(exp);
                }
            ...
            it(
                'should complete pseudo-element ' + str + ' after CSS native pseudo-class with prefix: ' + prefix + ' ',
                async () => {
                    const rng = createRange(9, 12, 9, 12 + i);

                    const asserter = await asserters
                        .getCompletions('pseudo-elements/default-import-with-native-class.st.css', prefix);
                    const exp: Array<Partial<Completion>> = [];
                    exp.push(createCompletion(str, rng));
                    asserter.suggested(exp);
                }
