    ...
    describe('Imported elements', () => {
        describe('Classes', () => {
            ...
            it('should return definition of imported class from 3rd party module', async () => {
                const defs = await asserters.getDefinition('definitions/imported-class-3rd-party.st.css');
                expect(defs.length).to.equal(1);
                const def = defs[0];
        ...
        describe('Vars', () => {
            ...
            it('should return definition of 3rd party var in -st-named', async () => {
                const defs = await asserters.getDefinition('definitions/3rd-party-var-named.st.css');
                expect(defs.length).to.equal(1);
                const def = defs[0];
                expect(def.uri).to.equal(
                    path.join(CASES_PATH, '../node_modules/fake-stylable-package/stylesheet.st.css')
        ...
        describe('States', () => {
            let callLocs = [
                ...
                { filePath: 'definitions/states-very-deep.st.css', pos: new ProviderPosition(10, 16) }
            ];
