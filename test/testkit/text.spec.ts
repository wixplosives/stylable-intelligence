...
describe("text test tools", function () {
    describe("offsetToLspPos", function () {
        ...
        for (let i = 0; i < linesArr.length; i++) {
            it(`line: ${i}, char: 0`, () => {
                expect(offsetToLspPos(totalLinesOffset(i), linesJoined), 'tsToLspPos').to.eql({line: i, character: 0});
            });
            if (linesArr[i].length) {
                it(`line: ${i}, char: 1`, () => {
                    expect(offsetToLspPos(totalLinesOffset(i) + 1, linesJoined), 'tsToLspPos').to.eql({
                        line: i,
                        character: 1
                    });
                ...
                if (linesArr[i].length > 1) {
                    it(`line: ${i}, char: ${linesArr[i].length}`, () => {
                        expect(offsetToLspPos(totalLinesOffset(i + 1) - 1, linesJoined), 'tsToLspPos').to.eql({
                            line: i,
    ...
    describe("getRangeAndText", function () {
        it("on empty text", function () {
            expect(getRangeAndText('||')).to.eql({
                text: '',
                range: {start: {line: 0, character: 0}, end: {line: 0, character: 0}}
            });
        });
        it("on zero pipes", function () {
            expect(() => getRangeAndText('sdfujsad')).to.throw(Error);
        });
        it("on one pipe", function () {
            expect(() => getRangeAndText('sdfu|jsad')).to.throw(Error);
        });
        it("on three pipe", function () {
            expect(() => getRangeAndText('sdfu|js|a|d')).to.throw(Error);
        });
        it("on range padded with characters on a single line", function () {
